...
Having discussed the dependency assumptions and the different forms of time series data.
...
\label{SectionTSCA}
This section introduces different types of TSCAs and discuss the various techniques that each type apply.
...
The $F_{\beta}$ score is an evaluation measure which combines both accuracy and earliness in one equation.
...
$F_{\beta}$ score definition is:
\begin{definition}
    \label{DefinitionHM}
    $F_{\beta} = (1 + \beta^2)\frac{\text{accuracy (1 - earliness)}}{\beta^2 \text{accuracy + (1 - earliness)}}$
\end{definition}

A special case of the $F_{\beta}$ score is the Harmonic mean (HM) or the $F_{1}\-score$.
...
For the scope of our framework, we will also consider the weighted average $F_{\beta}$ score to evaluate our classifiers.
